...
export class ChatController {
    async createConversation(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const conversation = await chatService.createConversation(req.user!.tenantId!, req.body, req.user!.employeeId!);
    ...
    async getConversations(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const result = await chatService.getConversations(req.user!.tenantId!, req.user!.employeeId!, req.query as any);
    ...
    async getConversation(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const conversation = await chatService.getConversation(req.params.id, req.user!.tenantId!);
