...
router.post('/', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_CREATE), validate(createProjectSchema), projectsController.create.bind(projectsController));
...
router.post('/from-quote/:quoteId', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_CREATE), validate(quoteIdSchema), projectsController.createFromQuote.bind(projectsController));
...
router.put('/:id', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), validate(updateProjectSchema), projectsController.update.bind(projectsController));
...
router.patch('/:id/stage', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), validate(stageUpdateSchema), projectsController.updateStage.bind(projectsController));
router.patch('/:id/status', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), validate(statusUpdateSchema), projectsController.updateStatus.bind(projectsController));
router.patch('/:id/assign', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), validate(assignManagerSchema), projectsController.assignProjectManager.bind(projectsController));
...
router.get('/:id/materials', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_VIEW), validate(projectIdSchema), projectsController.getMaterials.bind(projectsController));
router.post('/:id/materials', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), validate(genericBodySchema), projectsController.addMaterial.bind(projectsController));
router.put('/:id/materials/:materialId', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(nestedMaterialIdSchema), validate(genericBodySchema), projectsController.updateMaterial.bind(projectsController));
router.delete('/:id/materials/:materialId', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_DELETE), validate(nestedMaterialIdSchema), projectsController.deleteMaterial.bind(projectsController));
router.post('/:id/materials/from-quote', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), projectsController.importMaterialsFromQuote.bind(projectsController));

router.get('/:id/labor', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_VIEW), validate(projectIdSchema), projectsController.getLabor.bind(projectsController));
router.get('/:id/labor/summary', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_VIEW), validate(projectIdSchema), projectsController.getLaborSummary.bind(projectsController));
router.post('/:id/labor', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), validate(genericBodySchema), projectsController.addLabor.bind(projectsController));
router.put('/:id/labor/:laborId', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(nestedLaborIdSchema), validate(genericBodySchema), projectsController.updateLabor.bind(projectsController));
router.delete('/:id/labor/:laborId', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_DELETE), validate(nestedLaborIdSchema), projectsController.deleteLabor.bind(projectsController));
...
router.get('/:id/crews', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_VIEW), validate(projectIdSchema), projectsController.getCrews.bind(projectsController));
router.post('/:id/crews', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), validate(genericBodySchema), projectsController.assignCrew.bind(projectsController));
router.put('/:id/crews/:assignmentId', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(nestedAssignmentIdSchema), validate(genericBodySchema), projectsController.updateCrew.bind(projectsController));
router.delete('/:id/crews/:assignmentId', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_DELETE), validate(nestedAssignmentIdSchema), projectsController.removeCrew.bind(projectsController));
...
router.get('/:id/photos', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_VIEW), validate(projectIdSchema), projectsController.getPhotos.bind(projectsController));
router.post('/:id/photos', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), validate(genericBodySchema), projectsController.uploadPhoto.bind(projectsController));
router.put('/:id/photos/:photoId', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(nestedPhotoIdSchema), validate(genericBodySchema), projectsController.updatePhoto.bind(projectsController));
router.delete('/:id/photos/:photoId', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_DELETE), validate(nestedPhotoIdSchema), projectsController.deletePhoto.bind(projectsController));
...
router.get('/:id/change-orders', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_VIEW), validate(projectIdSchema), projectsController.getChangeOrders.bind(projectsController));
router.post('/:id/change-orders', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), validate(genericBodySchema), projectsController.createChangeOrder.bind(projectsController));
router.put('/:id/change-orders/:coId', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(nestedChangeOrderIdSchema), validate(genericBodySchema), projectsController.updateChangeOrder.bind(projectsController));
router.patch('/:id/change-orders/:coId/approve', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(nestedChangeOrderIdSchema), projectsController.approveChangeOrder.bind(projectsController));

router.get('/:id/weather-delays', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_VIEW), validate(projectIdSchema), projectsController.getWeatherDelays.bind(projectsController));
router.post('/:id/weather-delays', requireLegacyRoofingModule, requirePermission(PERMISSIONS.PROJECTS_UPDATE), validate(projectIdSchema), validate(genericBodySchema), projectsController.addWeatherDelay.bind(projectsController));
