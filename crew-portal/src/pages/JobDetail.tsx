...
import { Loader2, ArrowLeft, MapPin, Phone, Mail, CheckSquare, Clock, Users, Send, FileText } from 'lucide-react';
...
export default function JobDetailPage() {
    ...
    const [tab, setTab] = useState<'info' | 'tasks' | 'notes'>('info');
    ...
    const handleStatusChange = async (status: string) => {
        ...
        try {
            await crewApi.updateJobStatus(id!, { status });
            setJob({ ...job, status });
    ...
    return (
        <div className="page">
            ...
            <div style={{ display: 'flex', gap: 0, marginBottom: 16, borderBottom: '1px solid #334155' }}>
                {(['info', 'tasks', 'notes'] as const).map(t => (
                    <button key={t} onClick={() => setTab(t)}
                        style={{ flex: 1, padding: '10px 0', background: 'none', border: 'none', color: tab === t ? '#0891B2' : '#64748B', fontSize: 13, fontWeight: 600, cursor: 'pointer', borderBottom: tab === t ? '2px solid #0891B2' : '2px solid transparent', fontFamily: 'var(--font)' }}>
                        {t === 'info' ? 'Details' : t === 'tasks' ? `Tasks (${job.tasks?.length || 0})` : `Notes (${job.jobNotes?.length || 0})`}
                    </button>
                ))}
            </div>
            ...
            {tab === 'info' && (
                <>
                        ...
                        <div className="detail-section">
                            ...
                            {client.primaryPhone && (
                                <div className="detail-row">
                                    ...
                                    <a href={`tel:${client.primaryPhone}`} style={{ color: '#0891B2', display: 'flex', alignItems: 'center', gap: 4, fontSize: 13 }}><Phone size={12} /> {client.primaryPhone}</a>
                                </div>
                            )}
                            {client.primaryEmail && (
                                <div className="detail-row">
                                    <span className="detail-label">Email</span>
                                    <a href={`mailto:${client.primaryEmail}`} style={{ color: '#0891B2', display: 'flex', alignItems: 'center', gap: 4, fontSize: 13 }}><Mail size={12} /> {client.primaryEmail}</a>
                                </div>
                            )}
                            {client.streetAddress && (
                                <div className="detail-row">
                                    <span className="detail-label">Address</span>
                                    <span className="detail-value" style={{ display: 'flex', alignItems: 'flex-start', gap: 4 }}><MapPin size={12} style={{ flexShrink: 0, marginTop: 2 }} /> {[client.streetAddress, client.city, client.province, client.postalCode].filter(Boolean).join(', ')}</span>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    <div className="detail-section">
                        ...
                        {job.code && <div className="detail-row"><span className="detail-label">Project Code</span><span className="detail-value">{job.code}</span></div>}
                        {job.startDate && <div className="detail-row"><span className="detail-label">Start Date</span><span className="detail-value">{new Date(job.startDate).toLocaleDateString()}</span></div>}
                        {job.endDate && <div className="detail-row"><span className="detail-label">End Date</span><span className="detail-value">{new Date(job.endDate).toLocaleDateString()}</span></div>}
                        <div className="detail-row"><span className="detail-label">Files</span><span className="detail-value">{job._count?.files || 0} file(s)</span></div>
                    </div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                        ...
                                        <div style={{ flex: 1 }}>
                                            ...
                                            <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                                                <span className={`badge badge-${task.priority?.toLowerCase() === 'high' || task.priority?.toLowerCase() === 'urgent' ? 'on-hold' : task.priority?.toLowerCase() === 'low' ? 'completed' : 'planning'}`}>{task.priority}</span>
                ...
                <div>
                    ...
                    <div className="card" style={{ marginBottom: 16 }}>
                        <div className="input-group" style={{ marginBottom: 8 }}>
                            ...
                            <textarea className="textarea" placeholder="Write a note or update..." value={noteContent} onChange={e => setNoteContent(e.target.value)} rows={3} />
                        </div>
                    ...
                    {(!job.jobNotes || job.jobNotes.length === 0) ? (
                        <div className="empty-state"><FileText /><h3>No Notes</h3><p>Add the first note for this job</p></div>
                    ) : (
                        job.jobNotes.map((note: any) => (
                            <div key={note.id} className="note-card">
                                ...
                                <div className="note-header">
                                    <span className="note-author">{note.employee?.user?.firstName} {note.employee?.user?.lastName}</span>
