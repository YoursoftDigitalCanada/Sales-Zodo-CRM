...
import type { SolutionFeatureContent } from "@/data/solutionFeatureContent";
import { solutionFeatureNav } from "@/data/solutionFeatureContent";

function SectionShell({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <div className={`mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 ${className}`}>{children}</div>;
}

function toneClasses(color: SolutionFeatureContent["color"]) {
  if (color === "green") {
    return {
...
export function SolutionFeatureTemplate({ content }: { content: SolutionFeatureContent }) {
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
            ...
            <div className="mt-8 flex flex-col gap-4 sm:flex-row">
              ...
              <Button asChild size="xl" variant="heroOutline">
                <Link to={content.secondaryCtaHref}>{content.secondaryCta}</Link>
              </Button>
            </div>
          </motion.div>
      ...
      <section className="bg-background py-20 md:py-24">
        <SectionShell>
          <div className="max-w-3xl">
            <p className={cn("text-xs font-bold uppercase tracking-[0.24em]", tone.accent)}>How this audience uses ZODO</p>
            <h2 className="mt-4 text-4xl font-bold tracking-tight text-foreground md:text-5xl">
              The workflow adapts to the business model, while the stack stays connected.
            </h2>
          </div>
        ...
        <SectionShell className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          >
            ...
            <h2 className="mt-5 text-4xl font-bold tracking-tight text-white">
              What this solution looks like when it is fully connected.
            </h2>
            <div className="mt-6 space-y-3">
          ...
          >
            <p className={cn("text-xs font-bold uppercase tracking-[0.24em]", tone.accent)}>Next step</p>
            <h2 className="mt-4 text-3xl font-bold tracking-tight text-foreground md:text-4xl">
              Want to see how this solution fits the rest of your roofing workflow?
            </h2>
            <p className="mt-5 text-base leading-8 text-muted-foreground">
              ZODO is designed so each solution path still benefits from the same estimator, CRM, job, communication, and revenue system behind it.
            </p>
            <div className="mt-8 flex flex-col gap-4 sm:flex-row">
              <Button asChild size="xl" variant="accent">
                <Link to="/solutions">
                  View all solution pages
                  <ArrowRight className="h-4 w-4" />
                </Link>
              </Button>
              <Button asChild size="xl" variant="heroOutline">
